...
def similar_movies(movie_name,number_of_movies,movies,similarity):
    ...
    movie_names=[movies.iloc[i[0]].movie_title for i in movies_list]
    return movie_names
...
def recommend_movies(movie_name,number_of_movies,movies,similarity,similarity_genre,similarity_cast):
    movie_index = movies[movies['movie_title']==movie_name].index[0]

    # based on all factors
    movie_name_all=similar_movies(movie_name,number_of_movies,movies,similarity)
    ...
    movie_name_genre_based=[movies.iloc[i[0]].movie_title for i in movies_list_genre_rating_based]
    ...
    movie_name_cast_based=[movies.iloc[i[0]].movie_title for i in movies_list_cast_rating_based]

    recommend={
        "all_feature":movie_name_all,
        "genre_based":movie_name_genre_based,
        "cast_based":movie_name_cast_based
    }
...
def max_rated_movie(movie_id_list,movies):
    ...
    for movie_id in movie_id_list:
        if (list(movies[movies["movie_id"]==movie_id]["rating"])[0]>m):
            ...
            movie=movie_id
#         print(list(movies[movies["movie_id"]==movie_id].rating)[0])
...
def movie_recommendation_user_based(user_index,number_of_movies,user_watched_moives,similarity_users,movies,similarity):
    ...
    if len(movie_list)<number_of_movies:
...
while movie_name!="exit":
    ...
    number_of_movies=5
    recommendations=recommend_movies(movie_name,number_of_movies,movies,similarity,similarity_genre,similarity_cast)
